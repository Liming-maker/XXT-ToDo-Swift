...
public class Exam
{
    ...
    public bool IsCompleted
    {
        get
        {
            var done = new[] { "已完成", "已批阅", "已提交", "待批阅", "已互评", "已结束", "查看卷子" };
    ...
    public string RemainingTime(DateTime? at = null)
    {
        ...
        if (DeadlineDate is { } dDate)
        {
            ...
            if (diff.TotalSeconds <= 0)
            {
                ...
                var h = abs.Hours;
                var m = abs.Minutes;
                return d > 0 ? $"已超期{d}天{h}小时" : h > 0 ? $"已超期{h}小时{m}分钟" : $"已超期{m}分钟";
            }
            var days = (int)diff.TotalDays;
            var hours = diff.Hours;
            var mins = diff.Minutes;
            return days > 0 ? $"剩余{days}天{hours}小时" : hours > 0 ? $"剩余{hours}小时{mins}分钟" : $"剩余{mins}分钟";
        }
    ...
    private static string ParseRemainingText(string raw)
    {
        ...
        if (h >= 24) { d += h / 24; h %= 24; }
        return d > 0 ? $"剩余{d}天{h}小时" : h > 0 ? $"剩余{h}小时{m}分钟" : m > 0 ? $"剩余{m}分钟" : raw;
    }
}
